...
uint8_t packetCounter														 __SECTION_ZERO("retention_mem_area0");
...
static uint16_t gpadc_sample_to_mv(uint16_t sample) {
    ...
    uint32_t ref_mv = 3600 * (GetBits16(GP_ADC_CTRL2_REG, GP_ADC_ATTN) + 1);
...
//		packetCounter++;
		...
		for (int i = 0; i < 100; i++) {
					uint16_t output = gpadc_read(); // Get uint16_t ADC reading
          out[i] = output;
    }
